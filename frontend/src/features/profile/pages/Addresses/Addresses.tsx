...
import { countries } from 'countries-list';
import { useAppDispatch, useAppSelector } from '../../redux/hooks';
// Assuming fetchAddresses is an action you will define in your addressSlice
...
const Addresses: React.FC = () => {
  const dispatch = useAppDispatch();
  // Destructure addresses, loading, and error from the address state in Redux
  const { items: addresses, loading, error } = useAppSelector((state) => state.address);

  const [showModal, setShowModal] = useState(false);
  const [editingAddress, setEditingAddress] = useState<Address | null>(null);
  const [formData, setFormData] = useState<Omit<Address, 'id'>>({ // Omit 'id' for new addresses
    type: 'home',
    name: '',
    phone: '',
    street: '',
  ...
  // --- NEW: Fetch addresses on component mount ---
  useEffect(() => {
    // Dispatch the action to fetch all addresses when the component loads
    dispatch(fetchAddresses());
  }, [dispatch]); // Add dispatch to dependency array to satisfy ESLint, though it's stable
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingAddress) {
      // When editing, merge existing ID with form data and dispatch modifyAddress
      dispatch(modifyAddress({ ...editingAddress, ...formData } as Address));
    } else {
      // When adding, dispatch createAddress with the form data
      dispatch(createAddress(formData));
    }
  ...
  const handleEdit = (address: Address) => {
    ...
    // Populate form data with the existing address details
    setFormData({
      type: address.type,
      name: address.name,
      phone: address.phone,
  ...
  const handleCloseModal = () => {
    ...
    // Reset form data to its initial empty state for a new address
    setFormData({
      type: 'home',
      name: '',
      phone: '',
  ...
  // Display loading state from Redux
  if (loading) return <div className="loading-spinner">Loading addresses...</div>;
  // Display error state from Redux
  if (error) return <div className="error-message">{error}</div>;
