// import type { createSlice, createAsyncThunk, PayloadAction } from '@reduxjs/toolkit';
// import { NotificationState, Notification, NotificationFilters, MarkAsReadRequest, DeleteNotificationRequest } from '../types/notification.types';
...
const notificationSlice = createSlice({
  ...
  reducers: {
    ...
    updateNotificationLocally: (state, action: PayloadAction<{ id: string; updates: Partial<Notification> }>) => {
      const { id, updates } = action.payload;
      const notification = state.notifications.find((n: Notification) => n.id === id); // Explicitly type 'n'
      if (notification) {
        Object.assign(notification, updates);
      }
    }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchNotifications.fulfilled, (state, action) => {
        ...
        state.pagination = { page, limit, total, hasMore };
        state.unreadCount = notifications.filter((n: Notification) => !n.isRead).length; // Explicitly type 'n'
      })
      .addCase(fetchNotifications.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string;
      ...
      .addCase(markNotificationsAsRead.fulfilled, (state, action) => {
        const notificationIds = action.payload;
        state.notifications.forEach((notification: Notification) => { // Explicitly type 'notification'
          if (notificationIds.includes(notification.id)) {
            notification.isRead = true;
          }
        });
        state.unreadCount = state.notifications.filter((n: Notification) => !n.isRead).length; // Explicitly type 'n'
      })
      .addCase(markNotificationsAsRead.rejected, (state, action) => {
        state.error = action.payload as string;
      })
      ...
      .addCase(deleteNotifications.fulfilled, (state, action) => {
        const notificationIds = action.payload;
        state.notifications = state.notifications.filter((n: Notification) => !notificationIds.includes(n.id)); // Explicitly type 'n'
        state.unreadCount = state.notifications.filter((n: Notification) => !n.isRead).length; // Explicitly type 'n'
        state.pagination.total = Math.max(0, state.pagination.total - notificationIds.length);
      })
      .addCase(deleteNotifications.rejected, (state, action) => {
